...
function setToken(token, email) {
  ...
  badge.textContent = email.split('@')[0].toUpperCase();
...
async function startScan() {
  ...
  if (scanType === 'text') {
    ...
    if (authToken) {
      ...
      runWebSocketScan(text);
    }
  } else if (scanType === 'image') {
    const file = document.getElementById('image-input').files[0];
    if (!file) return alert('Please select an image.');
    if (!authToken) return alert('Please login to scan images.');
    await runImageScan(file);
  } else if (scanType === 'url') {
    const url = document.getElementById('url-input').value.trim();
    if (!url) return alert('Please enter a URL.');
    if (!authToken) return alert('Please login to scan URLs.');
...
const AURA_RESPONSES = {
  ...
  'login': 'Click LOGIN in the top navigation to authenticate. Logged-in users get scan history, image scanning, and URL analysis.',
